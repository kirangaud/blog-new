...
router.get('/master', function(req, res, next) {
    if (req.session.user) {
        res.render('categoryMaster', { "username": req.session.user, title: 'Express' });
    } else {
        res.render("login");
    }
});
...
router.post('/data-grid-view', function(req, res, next) {
    // if (req.session.user) {
        ...
        q.all(baseExport.grid("category", columns, page, search_by, sort, order, query, parseInt(limit), start, draw, "", table_format)).then(function(result) {
            res.json(result);
        });
    /*} else {
        return res.json(500, {
