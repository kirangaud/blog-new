...
Module Packages
*********************************************/
var controller  = require('../controllers/webBannerController.js'),
    baseExport  = require('../baseExporter');

router.get('/master', function(req, res, next) {
    if (req.session.user) {
        res.render('webBannerMaster', { "username": req.session.user, title: 'Express' });
    } else {
        res.render("login");
    }
});
...
router.post('/data-grid-view', function(req, res, next) {
    // if (req.session.user) {
        ...
        q.all(baseExport.grid("webBanner", columns, page, search_by, sort, order, query, parseInt(limit), start, draw, "", table_format)).then(function(result) {
            res.json(result);
        });
    /*} else {
        return res.json(500, {
